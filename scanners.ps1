...
$fields = @(
    ...
    @{name = "Total Files"; value = [string]$totalFiles; inline = $true},
    @{name = "Verified"; value = [string]$verifiedMods.Count; inline = $true},
    @{name = "Unknown"; value = [string]$unknownMods.Count; inline = $true},
    @{name = "Suspicious"; value = [string]$suspiciousMods.Count; inline = $true},
    @{name = "Bypass"; value = [string]$bypassMods.Count; inline = $true},
...
if ($suspiciousMods.Count -gt 0 -or $bypassMods.Count -gt 0) {
    ...
    foreach ($mod in $bypassMods) { $cheatList += "- $($mod.FileName)" }
    
    $cheatFields = $fields.Clone()
    $cheatFields += @{name = "Suspicious Files"; value = ($cheatList -join [System.Environment]::NewLine); inline = $false}
